...
class TreeNode():
    def __init__(self, node_idx, fa_node, c, h, L, R, is_leaf):
        ...
        self.children_list = []         # (R, c): 为了在查询时用二分快速定位到孩子
        
        self.tp_positions = {}        # 用于pushup
        self.need_build = set()
        self.char_buckets = {}
        self.first_pos = {}
...
class SuffixTree():
    """优化：祖先节点保存pos和buc信息，从而解决为了防止叶子节点空而引入额外节点的方法，
       应该能减少大量空间
       查询时，先跑到叶子节点，如果该节点没有该字符，就一直跳父亲，直到有或到根
       
        将suffixtrie改成suffixtree，采用逐层合并节点的方法，但这就需要原数据
        **！目前不支持多模式！**
    """
    ...
    def next_char(self, suffix_idx, p):
        ...
        if self.building_steps[suffix_idx] > len(self.strings[idx]):
            return '!empty!', 0, 0
        # if pos >= len(self.strings[idx]):       # EOS之后就返回了，没必要再构建了
            # return '!empty!', 0, 0
        # else:
    ...
    def batched_insert(self):
        """成批的插入（bfs插入），这样可以边插入边剪枝，否则dfs插入会空间不够
        """
        que = Queue()
        # (node, height, L, R, already_EOS)  高度从0开始，[L, R)，路径上是否已经出现EOS了，如果出现了，就不用维护两个最值
    ...
    def __dfs_occ(self, cur_node, idx, qc):
        if cur_node.is_leaf:
            ...
            return count
        # print(cur_node.children_list)         # 朴素遍历儿子

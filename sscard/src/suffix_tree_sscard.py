...
class Spline():
    """refers to https://github.com/learnedsystems/RadixSpline
    """
...
class TreeNode():
    ...
    def unique_children_list(self):
        seen = {}
        for item in reversed(self.children_list):
            key = item[1]
            if key not in seen:
                seen[key] = item
        self.children_list = list(seen.values())[::-1]
...
class SuffixTree():
    ...
    def batched_insert(self):
        ...
        while que.empty() == False:
            ...
            cur_node.unique_children_list()
                
    
    def get_L(self, x):
        # assert x not in self.BUC
        # self.BUC[x] = 1
    ...
    def get_L_rank_for_range(self, cur, L, R):
        for i in range(L, R):
            self.get_L_rank_for_char(cur, i)
 
    
    def get_L_rank_for_char(self, cur, idx):
        # for i in range(cur.L, cur.R):
        c = self.get_L(idx)
        self.total_cnt[c] += 1
        append_to_dict(cur.tp_positions, c, (idx, self.total_cnt[c]))
    ...
    def build_or_push_up_new(self, cur, cm=10):
        ...
        # for c, positions in cur.tp_positions.items():
            if c not in cur.tp_positions:
                ...
                positions = cur.tp_positions[c]
            p_len = len(positions)
            
            cur_node_need_build = cur.need_build
            if c in cur.need_build and (p_len >= cm or cur.fa is None) and p_len > 0:
                assert c in cur.tp_positions
                if self.regression_method == "spline":                    
                    cur.char_buckets[c] = spline_regression(positions, self.error_bound)
                    cur.first_pos[c] = positions[0][0]
                    # cur.first_pos[c] = cur.tp_positions[c][0][0]
                    
                elif self.regression_method == "linear":
                    cur.char_buckets[c] = linear_regression(positions, self.bucket)
                    cur.first_pos[c] = positions[0][0]
    ...
    def learn_fun_with_pushup(self, cur):
        if cur.is_leaf:
            ...
            self.get_L_rank_for_range(cur, cur.L, cur.R)
            for c in self.C:
        ...
        las = cur.L
        for (R, c) in cur.children_list:
            new_node = cur.char_to_children[c]
        # for c, new_node in cur.char_to_children.items():
            
            if new_node.L > las:
                # print("!!", las, new_node.L)
                self.get_L_rank_for_range(cur, las, new_node.L)
            las = R
    ...
    def __dfs_occ(self, cur_node, idx, qc):
        if cur_node.is_leaf:
            ...
            return count
        # print(cur_node.children_list)         # brute force
        ...
        pos = bisect.bisect_right(cur_node.children_list, (idx, chr(0x10FFFF)))

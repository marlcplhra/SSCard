...
def incremental_painter_load_time(dname):
    filepath = "../figures/"
    filename = 'inc_' + dname + "_load_time"
    if dname == "DBLP_AN":
        x_ticks = []
        # x_ticks = [10000, 20000, 30000, 40000, 50000]
        # x_labels = ['1', '2', '3', '4', '5']
        m1_q_error, m2_q_error, _, _, rebuild_points, _, _, m1_load_t, m2_load_t = load_inc_results_new(f"../inc_exp/{dname}/SSCard_cache250000.0_5_with_pattern_trie_30000/output_load2.log")
        
        m1_load_t = [0, 0, 0] + m1_load_t
        m2_load_t = [0, 0, 0] + m2_load_t
    ...
    m1_load_t = [x * 1000 for x in m1_load_t]
    m2_load_t = [x * 1000 for x in m2_load_t]
    # print(len(m1_load_t))
    # print(len(m2_load_t))
    
    plt.ylabel("Load time (ms)", fontsize=18)
    plt.xlabel(r'# Data strings($ \times 10^4$)', fontsize=18)
    plt.plot(X, m1_load_t, label="Single-SSCard update strategy", marker='o')
    plt.plot(X, m2_load_t, label = "Multiple-SSCard update strategy", marker='s')
...
def main():
    ...
    incremental_painter_load_time("DBLP_AN")

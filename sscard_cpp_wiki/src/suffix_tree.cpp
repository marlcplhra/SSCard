...
TreeNode::TreeNode(const std::shared_ptr<TreeNode>& _fa, char32_t _c_fa, int _h, int _L, int _R, bool _leaf): fa(_fa), c_fa(_c_fa), h(_h), L(_L), R(_R), leaf(_leaf)
{
    occ_times = 0;
    children_list.clear();
}
...
void TreeNode::add_to_tp_positions(char32_t c, Pos_rk x)
{
...
void SuffixTree::buildSuffixTree(std::vector<char32_t> sigma, int sum_len, Idx_pos sorted_rotations[], char32_t* data_strings[])
{
    this->sigma = sigma;
...
char32_t SuffixTree::next_char(int suffix_idx, int p, Idx_pos sorted_rotations[], char32_t* data_strings[])
{
    building_steps[suffix_idx] += 1;
    int idx = sorted_rotations[suffix_idx].idx;
    int len = calc_len(data_strings[idx]);
...
std::shared_ptr<TreeNode> SuffixTree::create_node(std::shared_ptr<TreeNode> cur_node, char32_t c, int L, int R)
{
...
void SuffixTree::batched_insert(int sum_len, Idx_pos sorted_rotations[], char32_t* data_strings[])
{
    std::queue<Queue> q;
    ...
    while(!q.empty())
    {
        ...
        char32_t cur_c = next_char(L, h, sorted_rotations, data_strings);
        int beg = L;
        for(int i = L + 1; i < R; ++i)
        {
            char32_t c = next_char(i, h, sorted_rotations, data_strings);
            if(c != cur_c)
            {
                if(cur_c != '\0')
                {
...
char32_t SuffixTree::get_L(char32_t *data_strings[], Idx_pos r)
{
...
void SuffixTree::get_L_rank_for_chars(std::shared_ptr<TreeNode> cur, Idx_pos sorted_rotations[], char32_t* data_strings[])
{
    int len = cur->R - cur->L;
    for(int i = 0; i < len; ++i)
    {
        char32_t c = get_L(data_strings, sorted_rotations[cur->L + i]);
...
void SuffixTree::learn_fun_with_pushup(std::shared_ptr<TreeNode> cur, Idx_pos sorted_rotations[], char32_t* data_strings[])
{
    if(cur->leaf)
    {
        get_L_rank_for_chars(cur, sorted_rotations, data_strings);
        for(char32_t i: sigma) cur->need_build[i] = 1;
...
void SuffixTree::build_spline(std::shared_ptr<TreeNode> cur, char32_t c)
{
...
TreeResults SuffixTree::dfs_occ_in_tree(std::shared_ptr<TreeNode> cur, int x, int len, char32_t* p)
{
    if(cur->leaf || x == len)
...
TreeResults SuffixTree::count_with_tree(char32_t* p)
{
    int m = calc_len(p);
...
int SuffixTree::get_rk_from_fa(std::shared_ptr<TreeNode> cur, int idx, char32_t c)
{
    while(!cur->fun.count(c) && !cur->fa.expired()) 
        cur = cur->fa.lock();
    if(!cur->fun.count(c)) return 0;
    assert(c < Const::MAX_CHAR);
...
int SuffixTree::dfs_estimate_rk(std::shared_ptr<TreeNode> cur, int idx, char32_t c)
{
    if(idx <= 0) return 0;
    if(cur->leaf) return get_rk_from_fa(cur, idx, c);
    std::vector<R_child>& Ch = cur->children_list;
    auto child = std::upper_bound(Ch.begin(), Ch.end(), (R_child){idx, Const::MAX_CHAR});

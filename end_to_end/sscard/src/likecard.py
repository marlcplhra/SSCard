...
from suffix_tree_likecard import SuffixTree

from suffix_tree_likecard import spline_regression
...
def multi_bwt_radix_sort(strings, args):
    ...
    # print(ret[:10])
    tree_path = os.path.join(args.load_path, "tree_final" + str(args.l))
    if False and args.load_L and os.path.exists(tree_path):
        mytree = load(tree_path)
        mytree.regression_method = args.fitting
        mytree.error_bound = args.e
        print("Loading tree finished.")
    else:
        mytree = SuffixTree(sorted_rotations, data_strings, args)
        if args.load_L:
            save(tree_path, mytree)
...
class LikeCard():
    ...
    def __init__(self, data, args):
        ...
        if args.no_tree:
            self.data, self.regression_list, self.L_time, self.spline_time, self.fitting_function_num = multi_bwt_radix_sort_no_tree(data, args)
            self.L_len = len(self.data)
            self.C, _ = calc_C(self.data) 
            del self.data
            return
        ...
        # if not hasattr(self.myTree_multi, "bucket"):

...
def create_sqlite_db_repeat(dataset_path, db_path, chunk_num):
    ...
    if not os.path.exists(db):
        ...
        c.execute('CREATE INDEX trans_pattern on pattern(trans)')
...
def solve_pattern_list(chunk_id, chunk, db_path, datasetsize, path_file_tosave, lock):
    # path = dataset_path.rsplit('/', 1)[0]
...
def main(dataset_path, db_path, list_of_patterns, path_file_tosave, datasetsize):
    ...
    # list_of_patterns = list_of_patterns[:1000]
    ...
    for i, chunk in enumerate(chunks):
        p = multiprocessing.Process(target=solve_pattern_list, args=(i, chunk, \
...
if __name__ == "__main__":
    ...
    dname = sys.argv[1]
    dataset_path = f'../../pg_exp/columns/{dname}/{dname}.txt'
    db_path = f'./Datasets_pg/{dname}/dbs_repeat/'
    like_patterns_path = f'../../pg_exp/columns/{dname}/{dname}_substring_counts_sampled.csv'
    file_to_save_ground = f'./Datasets_pg/{dname}/{dname}_train_ground_truth.txt'
    datasetsize = 0
    with open(dataset_path, "r") as f:
        for line in f.readlines():
            datasetsize += 1

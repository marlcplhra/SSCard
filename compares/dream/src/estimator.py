...
class Estimator(metaclass=ABCMeta):
    ...
    def estimate_latency_anlysis(self, test_data):
        ...
        y_data = []
        latencies = []
        tqdm_test = tqdm(test_data, total=len(test_data))
        for test_qry in tqdm_test:
...
def learn_Deep_Learning_model(cm, est_class: Type[Estimator], data, is_rewirte=False, logger=None, exp_key=None, overwrite=None, n_qry=None, n_prfx=None, n_update=None, analysis=None, analysis_latency=None):
    ...
    """
    print(analysis, analysis_latency)
    ...
    if analysis and not analysis_latency:
        ...
        strings = df['s_q'].to_list()
        y = df['true_count'].to_list()
        y_hat = df['est_count'].to_list()
        
        for i, (q, tru, est) in enumerate(zip(strings, y, y_hat)):
            tru = max(tru, 1)
            est = max(est, 1)
            q_error_ = max(tru / est, est / tru)
            if q_error_ > 10000:
                print(q, tru, est, q_error_)
        
        # y_hat = [max(x, 1) for x in y_hat]
        MAE = np.mean([math.fabs(y1 - y2) for (y1, y2) in zip(y, y_hat)])
        # print(y[:10], y_hat[:10])
        print("MAE:", MAE)
        q_error = ut.q_error(y, y_hat)
        q_error_dict = {}
        # test_q_error = [10, 20, 25, 30, 40, 50, 60, 70, 75, 80, 90, 95, 99, 100]
        test_q_error = [50, 70, 90, 99, 100]
        for percentile in test_q_error:
            q_error_dict[f"q{percentile}"] = ut.q_error(y, y_hat, f"q{percentile}")
        print("mean q-error", ut.q_error(y, y_hat, "mean"))
        print(q_error_dict)
    ...
    print("Model size:", model_size / 1024 / 1024)
    ...
    # len_MAE = {}
    # aaaa = []
    # assert len(test_data) == len(y_hat)
    # cnt = 0
    # for i, ((p, _, _), n, c) in enumerate(zip(test_data, y, y_hat)):
    #     if n < 1:
    #         n = 1
    #     if c < 1:
    #         c = 1
    #     cnt += 1
    #     lp = len(p)
    #     if lp not in len_q_error:
    #         len_q_error[lp] = []
    #         len_MAE[lp] = []
    #     # print(p, lp, n, c)
    #     aaaa.append(max(c / n, n / c))
    #     len_q_error[lp].append(max(c / n, n / c))
    #     len_MAE[lp].append(math.fabs(c - n))
    
    # for i in range(1, 21):
    #     if i not in len_q_error: continue
    #     len_q_error[i] = np.mean(len_q_error[i])
    #     len_MAE[i] = np.mean(len_MAE[i])
    # print("Test data len:", cnt)
    # print("!!!!", np.mean(aaaa))
    
    # filepath = "../../../../data/" + dname + "/results_per_length"
    # if not os.path.exists(filepath):
    #     os.mkdir(filepath)
    # filename = os.path.join(filepath, "LBS.csv")
    # with open(filename, "w") as f:
    #     f.write("length,Q_error,MAE\n")
    #     for i in range(1, 21):
    #         if i not in len_q_error: continue
    #         f.write(str(i) + ',')
    #         f.write(str(len_q_error[i]))
    #         f.write(",")
    #         f.write(str(len_MAE[i]))
    #         f.write("\n")
        
    # q_error = ut.q_error_np(y, y_hat, eps=10.)
    q_error = ut.q_error(y, y_hat)
    q_error50 = ut.q_error(y, y_hat, "q50")
    q_error90 = ut.q_error(y, y_hat, "q90")
    q_error99 = ut.q_error(y, y_hat, "q99")
    q_errormax = ut.q_error(y, y_hat, "max")
    MAE = np.mean([math.fabs(y1 - y2) for (y1, y2) in zip(y, y_hat)])
    # print(y[:10], y_hat[:10])
    print("MAE:", MAE)
    print(f"average q-error: {q_error:.3f}")
    print(f"[50, 90, 99, max]: {q_error50:.3f}, {q_error90:.3f}, {q_error99:.3f}, {q_errormax:.3f}")
    print("building time:", duration)
    print("Estimation time:", est_duration)
    print("Estimation time(queries only):", np.sum(latencies), len(latencies))

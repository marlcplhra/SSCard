...
def prepare_dataset(folder_path, dataset_prefix):
    ...
    functions = [summary_data_structures.get_all_substrings]
    # function_desc = ["prefix", "suffix", "substring"]
    function_desc = ["substring"]
    input_file_name = folder_path + dataset_prefix + ".csv"
    for index, fn in enumerate(functions):
        count_file_name = folder_path + dataset_prefix + "_" + function_desc[index] + "_counts_test_only.csv"
        triplet_file_name = folder_path + dataset_prefix + "_" + function_desc[index] + "_triplets_test_only.csv"
        ...
        end_tri = time.time()
        sel_duration = end_sel - sta_sel
        tri_duration = end_tri - sta_tri
        print("Generating ground truth time:", sel_duration)
        print("Generating triplets time:", tri_duration)
        print("Total time:", sel_duration + tri_duration)
...
if __name__ == "__main__":
    ...
    prepare_dataset("../../datasets/DBLP_AN/", "DBLP_AN")
    prepare_dataset("../../datasets/IMDB_AN/", "IMDB_AN")
    prepare_dataset("../../datasets/IMDB_MT/", "IMDB_MT")
    prepare_dataset("../../datasets/TPCH_PN/", "TPCH_PN")
    prepare_dataset("../../datasets/WIKI/", "WIKI")
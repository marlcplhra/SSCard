...
StringFrequencyConfigs = namedtuple('StringFrequencyConfigs',
    ['string_list_file_name', 'selectivity_file_name', 'triplets_file_name', 'selectivity_file_name_test'])
...
SelectivityEstimatorConfigs = namedtuple('SelectivityEstimatorConfigs',
    ...
    'min_val', 'max_val',
    'embedding_model_file_name', 'selectivity_model_file_name', 'estimation_file'])
...
def compute_qerrors(preds=None, targets=None, min_val=0.0, max_val=1.0):
    ...
    for i in range(len(targets)):
        if (preds[i] > targets[i]).cpu().data.numpy()[0]:
            qerror.append((preds[i] / targets[i]).cpu().data.numpy()[0])
        else:
            qerror.append((targets[i] / preds[i]).cpu().data.numpy()[0])
    return qerror

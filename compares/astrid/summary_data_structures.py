...
class SummaryDataStructure:
    ...
    def get_triplets(self, random_seed=1234, num_triplets_per_node=4):
        ...
        # nodes = [node for node in LevelOrderIter(self.root_node) if not node.is_root]
        # frequencies = [node.frequency for node in nodes]
        # # 计算频率的90%分位数
        # top_10_percent_threshold = np.percentile(frequencies, 90)
        # # 筛选出频率值大于等于这个阈值的节点
        # top_10_percent_nodes = [node for node in nodes if node.frequency >= top_10_percent_threshold]        

        all_triplets = []

        for node in LevelOrderIter(self.root_node):
        # for node in top_10_percent_nodes:

...
class StringDatasetHelper:
    ...
    # !!!!only for WIKI!!!!
    # def extract_alphabets(self, file_name, max_char=200):
    #     f = open(file_name, "r")
    ...
    #     char_dict = dict()
    #     for rid, record in enumerate(lines):
    #         for char in record:
    #             if char not in char_dict:
    #                 char_dict[char] = 0
    #             char_dict[char] += 1
    #     char_set = list(zip(*sorted(char_dict.items(), key=lambda x: x[1], reverse=True)[:max_char]))[0]  # frequency order
    #     for idx, c in enumerate(char_set):
    #         self.alphabets[c] = idx + 2     # 0: [PAO] 1: [UNK]
        
    #     # self.alphabets = "".join(sorted(set().union(*lines)))
    #     self.alphabet_size = len(char_set) + 2
    ...
    #             indices.append(self.alphabets[c])
    #         else:
    #             indices.append(1)
    #     return indices
    ...
    def __init__(self, df, string_helper, embedding_model):
        ...
        with torch.no_grad():
            for string in self.strings:
                ...
                self.strings_as_tensors.append(self.embedding_model(string_as_tensor).cpu().numpy())
